...
const pr = (over = {}) => ({
  number: 42, title: 'add widget', state: 'OPEN', updatedAt: '2026-07-01T10:00:00Z',
  isDraft: false, statusCheckRollup: [{ name: 'build', status: 'COMPLETED', conclusion: 'FAILURE' }],
  reviewDecision: 'REVIEW_REQUIRED', latestReviews: [], mergeable: 'UNKNOWN',
  comments: [], headRefOid: 'sha1', ...over,
});
...
test('OPEN → MERGED emits `merged`, not `closed`', () => {
  ...
  const r = detectDeltas(base.snapshot, { pr: [pr({ state: 'MERGED', updatedAt: '2026-07-01T11:00:00Z' })], issue: [] });
...
test('mergeable UNKNOWN → MERGEABLE does NOT emit became-mergeable', () => {
  ...
  const r = detectDeltas(base.snapshot, { pr: [pr({ mergeable: 'MERGEABLE', updatedAt: '2026-07-01T11:00:00Z' })], issue: [] });
...
test('an identical PR (only array reorder) emits NO delta', () => {
  const base = detectDeltas(null, { pr: [pr({ statusCheckRollup: [
    { name: 'build', status: 'COMPLETED', conclusion: 'FAILURE' },
    { name: 'lint', status: 'COMPLETED', conclusion: 'SUCCESS' },
  ] })], issue: [] });
  const r = detectDeltas(base.snapshot, { pr: [pr({ statusCheckRollup: [
    { name: 'lint', status: 'COMPLETED', conclusion: 'SUCCESS' },
    { name: 'build', status: 'COMPLETED', conclusion: 'FAILURE' },
  ] })], issue: [] });
  assert.deepEqual(r.deltas, []);
...
test('draft → ready emits draft-ready', () => {
  ...
  const r = detectDeltas(base.snapshot, { pr: [pr({ isDraft: false, updatedAt: '2026-07-01T11:00:00Z' })], issue: [] });
  assert.ok(r.deltas[0].classes.includes('draft-ready'));
});

test('issue label removal emits relabeled', () => {
  const issue = { number: 7, title: 'bug', state: 'OPEN', updatedAt: '2026-07-01T10:00:00Z', labels: [{ name: 'worker' }, { name: 'backend' }], comments: [] };
  const base = detectDeltas(null, { pr: [], issue: [issue] });
  const r = detectDeltas(base.snapshot, { pr: [], issue: [{ ...issue, updatedAt: '2026-07-01T11:00:00Z', labels: [{ name: 'backend' }] }] });
  assert.ok(r.deltas[0].classes.includes('relabeled'));
});

test('a bare updatedAt bump with no specific signal emits `updated`', () => {
  const base = detectDeltas(null, { pr: [pr()], issue: [] });
  const r = detectDeltas(base.snapshot, { pr: [pr({ updatedAt: '2026-07-01T11:00:00Z' })], issue: [] });
...
test('omitted entity collection preserves that side of the snapshot', () => {
  const issue = { number: 7, title: 'bug', state: 'OPEN', updatedAt: '2026-07-01T10:00:00Z', labels: [], comments: [] };
...
test('capped comments plus updatedAt bump emits new-comments instead of updated', () => {
  const issue = { number: 7, title: 'bug', state: 'OPEN', updatedAt: '2026-07-01T10:00:00Z', labels: [], comments: Array.from({ length: 100 }, () => ({})) };
  const base = detectDeltas(null, { pr: [], issue: [issue] });
  const r = detectDeltas(base.snapshot, { pr: [], issue: [{ ...issue, updatedAt: '2026-07-01T11:00:00Z' }] });
