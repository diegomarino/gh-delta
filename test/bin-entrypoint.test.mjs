...
test('gh-delta-tick starts when invoked through an npm-style bin symlink', () => {
  const output = execFileSync(process.execPath, [symlinkedBin('gh-delta-tick.mjs'), '--help'], {
    encoding: 'utf8',
  });

  assert.match(output, /^Usage:\n {2}gh-delta-tick --repo/);
});
