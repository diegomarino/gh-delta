...
// see lib/cli.mjs), so it doubles as the delta's contextual metadata: PR
...
function diffEntity(kind, oldMap, objects, fpFn, classifyFn, { at, staleAfterMs } = {}) {
  ...
  for (const [key, oldItem] of Object.entries(oldMap)) {
    ...
    deltas.push({
      entity: kind,
      number: Number(key),
      // Last known context, minus title: we cannot confirm the current title
      // without a successful fetch, so it goes explicitly null rather than
      // repeating stale display text as if it were still authoritative.
      context: { ...oldItem.context, title: null },
      classes,
      missingTicks: ticks,
      from: oldItem,
      to: null,
