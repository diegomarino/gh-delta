...
function comparableFingerprint(fp) {
  ...
  if (!fp) return fp;
  let normalized = fp;
  if (Object.hasOwn(normalized, 'missing')) {
    const { missing: _missing, ...rest } = normalized;
    normalized = rest;
  }
  if (!Object.hasOwn(normalized, 'commentsOverflow')) {
    normalized = { ...normalized, commentsOverflow: false };
  }
  if (
    'ci' in normalized ||
