...
// object exists. PR deltas additionally carry the head branch name (symmetric
// with title); issues have no head branch. GitHub retains `headRefName` even
// after the branch is deleted, so it is effectively always present (`?? null`
// is defensive only). The missing/presumed-deleted branch has no current
// object, so it never calls this.
 ...
 * double opt-in (lib/cli.mjs), so both agree on exactly which threads and
...
export function threadReplyIncrements(oldThreads = [], newThreads = []) {
  ...
  for (const t of newThreads ?? []) {
    ...
    if (increment > 0) increments.push({ id: t.id, increment });
  }
...
function classifyPr(oldFp, fp) {
  ...
  // A push always moves headSha; this coexists with (does not replace) the
  ...
  if (oldFp.headSha !== fp.headSha) c.push('head-changed', 'updated');
