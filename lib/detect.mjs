...
function classifyPr(oldFp, fp) {
  ...
  if (fp.commentsOverflow && oldFp.commentsOverflow && fp.updatedAt !== oldFp.updatedAt) c.push('new-comments');
  return c;
}
...
function fingerprintChanged(a, b) {
  return JSON.stringify(stableValue(comparableFingerprint(a))) !== JSON.stringify(stableValue(comparableFingerprint(b)));
}
...
function diffEntity(kind, oldMap, objects, fpFn, classifyFn) {
  ...
  for (const obj of objects) {
    ...
    if (!oldFp) {
      ...
      deltas.push({ entity: kind, number: obj.number, title: obj.title, classes: ['new'], from: null, to: fp });
      continue;
    }
    ...
    deltas.push({ entity: kind, number: obj.number, title: obj.title, classes, from: oldFp, to: fp });
  }
  for (const [key, oldFp] of Object.entries(oldMap)) {
    ...
    deltas.push({ entity: kind, number: Number(key), title: '(missing from current fetch)', classes, from: oldFp, to: null });
  }
...
export function detectDeltas(oldSnapshot, current) {
  ...
  const prRes = Array.isArray(current.pr) ? diffEntity('pr', oldPr, current.pr, prFingerprint, classifyPr) : { deltas: [], nextMap: oldPr };
  const issueRes = Array.isArray(current.issue) ? diffEntity('issue', oldIssue, current.issue, issueFingerprint, classifyIssue) : { deltas: [], nextMap: oldIssue };
