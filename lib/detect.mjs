...
// Diff two sorted `{id, isResolved}` arrays for thread-identity set changes:
// ids newly unresolved (new thread, or an old one reopened) vs. ids newly
// resolved. Guarded on both sides being arrays: an old snapshot predating
// `threadStates` (or a fingerprint built with no thread data at all) must not
// be read as "every thread just disappeared".
export function threadSetDiff(oldStates, newStates) {
  if (!Array.isArray(oldStates) || !Array.isArray(newStates)) {
    return { addedIds: [], removedIds: [] };
  }
  const oldResolved = new Map(oldStates.filter((t) => t?.id).map((t) => [t.id, t.isResolved]));
  const addedIds = [];
  const removedIds = [];
  for (const t of newStates) {
    if (!t?.id) continue;
    const wasResolved = oldResolved.has(t.id) ? oldResolved.get(t.id) : null;
    if (!t.isResolved && wasResolved !== false) addedIds.push(t.id); // newly unresolved
    if (t.isResolved && wasResolved === false) removedIds.push(t.id); // newly resolved
  }
  addedIds.sort();
  removedIds.sort();
  return { addedIds, removedIds };
...
// Strip only the missing-lifecycle bookkeeping (`missing`/`missingTicks`) before
// feeding an old fingerprint to classifyFn on the reappearance path. Unlike
// `comparableFingerprint`, this deliberately keeps `threadStates` (and every
// other field) intact: classifyPr's thread-identity diff needs the old thread
// states to recognize a same-count swap that happened while the item was
// missing. This is used only to build classifyFn's `oldFp` argument, never for
// `deltaIdentity`/`fingerprintChanged`, so it cannot affect any delta id.
function stripMissingBookkeeping(fp) {
  if (!fp) return fp;
  const { missing: _missing, missingTicks: _missingTicks, ...rest } = fp;
  return rest;
}
...
function classifyPr(oldFp, fp) {
  ...
  // A push always moves headRefOid; this coexists with (does not replace) the
  // catch-all `updated` class further down so a caller can branch on "the
  // head moved" without losing the generic "something changed" signal.
  if (typeof oldFp.head === 'string' && oldFp.head !== fp.head) c.push('head-changed', 'updated');
  ...
  if (oldFp.state === 'OPEN' && fp.state === 'OPEN') {
    ...
    const { addedIds, removedIds } = threadSetDiff(oldFp.threadStates, fp.threadStates);
    const added = newUnresolved > oldUnresolved || addedIds.length > 0;
    const resolved = newUnresolved < oldUnresolved || removedIds.length > 0;
    if (added) c.push('unresolved-threads-added');
    if (resolved) c.push('unresolved-threads-resolved');
    if (!added && !resolved && newThreads !== oldThreads) c.push('review-threads-changed');
  }
  return c;
}
...
function fingerprintChanged(a, b) {
  ...
  if (JSON.stringify(stableValue(ca)) !== JSON.stringify(stableValue(cb))) return true;
  // threadDigest is deliberately excluded from comparableFingerprint (it must
  // never enter the delta id -- see comparableFingerprint's doc comment), so it
  // is consulted here as a second, independent trigger: a same-count thread
  // swap (one resolved, another reopened) leaves `ca`/`cb` equal but must still
  // be observed. Guarded on BOTH sides carrying a string digest: a snapshot
  // written before the digest existed must not have its first appearance read
  // as a change (same upgrade-safety rule as ADDITIVE_COMPARED_FIELDS above).
  if (typeof a?.threadDigest === 'string' && typeof b?.threadDigest === 'string') {
    return a.threadDigest !== b.threadDigest;
  }
  return false;
}
...
function diffEntity(kind, oldMap, objects, fpFn, classifyFn, { at, staleAfterMs } = {}) {
  ...
  for (const obj of objects) {
    ...
    if (!oldFp) {
      nextMap[key] = staleEnabled ? { ...fp, lastChangedAt: at, ticksSinceChange: 0 } : fp;
    ...
    if (wasMissing) {
      ...
      if (changedAfterReturn) {
        const specific = classifyFn(stripMissingBookkeeping(oldFp), fp);
    ...
    const changed = fingerprintChanged(oldFp, fp);
    if (
      staleEnabled &&
      oldFp.lastChangedAt !== undefined &&
      !Number.isFinite(Date.parse(oldFp.lastChangedAt))
    )
      throw new Error(`invalid persisted lastChangedAt for ${kind} #${key}`);
    if (!changed) {
      if (!staleEnabled) {
        // A normal detector tick must remain byte-identical for legacy
        // snapshots, but it must not erase stale bookkeeping that an earlier
        // explicit --stale-after tick established. It cannot advance that
        // bookkeeping without a threshold, so preserve it verbatim.
        nextMap[key] = {
          ...fp,
          ...(oldFp.lastChangedAt !== undefined ? { lastChangedAt: oldFp.lastChangedAt } : {}),
          ...(oldFp.ticksSinceChange !== undefined
            ? { ticksSinceChange: oldFp.ticksSinceChange }
            : {}),
          ...(oldFp.staleEmittedFor !== undefined
            ? { staleEmittedFor: oldFp.staleEmittedFor }
            : {}),
        };
        continue;
      }
      const lastChangedAt = oldFp.lastChangedAt ?? at;
      const ticksSinceChange =
        (Number.isSafeInteger(oldFp.ticksSinceChange) && oldFp.ticksSinceChange >= 0
          ? oldFp.ticksSinceChange
          : -1) + 1;
      const next = {
        ...fp,
        ...(lastChangedAt ? { lastChangedAt } : {}),
        ticksSinceChange,
        ...(oldFp.staleEmittedFor ? { staleEmittedFor: oldFp.staleEmittedFor } : {}),
      };
      if (
        staleAfterMs &&
        at &&
        fp.state === 'OPEN' &&
        atMs - Date.parse(lastChangedAt) >= staleAfterMs
      ) {
        const staleAt = new Date(atMs).toISOString().slice(0, 10);
        if (oldFp.staleEmittedFor !== staleAt) {
          next.staleEmittedFor = staleAt;
          deltas.push({
            entity: kind,
            number: obj.number,
            title: obj.title,
