...
const HELP_SPECS = {
    ...
    subcommands: [
      {
        ...
        summary:
          'Read-only inventory of the monitors that have run against a state directory. See gh-delta list --help.',
      },
    ],
    options: [
      OPTION_REPO,
  ...
  'gh-delta list': {
    ...
    version: PACKAGE_METADATA.version,
    summary: 'Read-only inventory of monitor snapshots in a state directory.',
    usage: 'gh-delta list [--state-dir <dir>] [--since <duration>] [--format json|text]',
    purpose:
      'List the monitors that have run against a state directory: repo, monitor id, entities, last run, and stored object counts, decoded from derived snapshot filenames. Read-only: never contacts GitHub and never creates, updates, or deletes snapshots. Only derived snapshots (--state-dir or the temp-dir default) are discoverable; explicit --state-file snapshots are not.',
    options: [
      {
        name: '--state-dir',
        valueName: 'dir',
        ...
        required: false,
        description:
          'State directory to inventory. Optional: defaults to the per-user temp state directory the detector uses when neither --state-file nor --state-dir is given.',
      },
      {
        name: '--since',
        valueName: 'duration',
    ...
    output: {
      ...
      description:
        'JSON output contains schemaVersion, command ("list"), stateDir, since (the echoed --since value or null), at, monitors, skippedFiles, and summary. Each monitor entry carries repo, monitorId, entities, stateFile, lastRun (snapshot meta.horizon when readable, file mtime otherwise), and prCount/issueCount (null with an error string when the snapshot is corrupt). Entries are sorted by lastRun, newest first. skippedFiles counts directory entries that are not derived snapshot files. summary is human-readable only; do not parse it.',
    },
    exitCodes: [
    ...
    safety: [
      ...
      'list never creates, updates, or deletes snapshots; it is safe to run while monitors tick.',
      'A corrupt snapshot is reported as an entry with an error field, not a process failure.',
    ],
    examples: [
      {
        description: 'List every monitor that uses a shared state directory.',
        command: 'gh-delta list --state-dir .gh-delta',
      },
      {
        description: 'List monitors that ran in the last 24 hours, as operator text.',
        command: 'gh-delta list --state-dir .gh-delta --since 24h --format text',
      },
      {
        description: 'Inspect the zero-config temp-dir monitors.',
        command: 'gh-delta list',
      },
    ],
  },
