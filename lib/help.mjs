...
const OPTION_OUTPOST_URL = {
  ...
  required: false,
  description:
    'Send one fire-and-forget HTTP POST per delta when exit code is 10. Body: {type, schemaVersion, deliveryId, seq, monitorId, detectedAt, delta}, where delta is the report delta verbatim. Dedupe rule: deliveryId is unique per send attempt (use it for processing idempotency, e.g. at-most-once handling of retried deliveries); delta.id is the content-addressed identity of the observed change (compare it against the last id seen per item to collapse duplicates reported by several monitors).',
};
...
const OPTION_OUTPOST_SECRET = {
  ...
  description:
    'Read the outpost HMAC secret from this environment variable and sign each POST per the Standard Webhooks spec: webhook-id (deliveryId), webhook-timestamp (epoch seconds), webhook-signature (v1,<base64 HMAC-SHA256 of "{id}.{timestamp}.{body}">). Requires --outpost-url; the argument is a variable name, never the secret value.',
};
...
const HELP_SPECS = {
  'gh-delta': {
    ...
    usage:
      'gh-delta [--repo <owner/name>] [--monitor-id <id>] [--state-file <path> | --state-dir <dir>] [--entities pr,issue] [--format json|text|compact|ndjson] [--summary-line] [--detail] [--summaries] [--full] [--enrich review,comments,threads,body,thread-replies] [--only-classes <classes>] [--ignore-classes <classes>] [--ignore-authors <logins>] [--settled] [--baseline-emit-state] [--log] [--outpost-url <url>] [--outpost-secret <ENV_VARIABLE_NAME>] [--outpost-timeout-ms <ms>] [--outpost-max-posts <n>] [--gh-timeout-ms <ms>] [--rate-limit-floor <n>] [--no-registry] [--lock-stale-ms <duration>] [--stale-after <duration>]',
    purpose:
    ...
    options: [
      ...
      },
      {
        ...
        required: false,
        description:
          'Attention filter: drop a delta whose summary has ciRollup pending or mergeable unknown. ciRollup none is settled.',
      },
      {
  ...
  'gh-delta wait': {
    ...
    options: [
      ...
      },
      {
        ...
        required: false,
        description:
          'Current PR summary condition; comma-separated values are alternatives and are evaluated on the first iteration. failedChecks is not accepted (it is a list, not a scalar); use --until ci-changed instead.',
      },
      {
        name: '--interval',
        valueName: 'duration',
  ...
  'gh-delta reset': {
    ...
    output: {
      ...
      targetFields: RESET_TARGET_FIELDS,
      description:
