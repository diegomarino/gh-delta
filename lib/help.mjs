...
const HELP_SPECS = {
  'gh-delta': {
    ...
    usage:
      'gh-delta --repo <owner/name> --monitor-id <id> (--state-dir <dir> | --state-file <path>) [--entities pr,issue] [--format json|text] [--outpost-url <url>]',
    purpose:
    ...
    options: [
      ...COMMON_DETECTOR_OPTIONS.slice(0, 6),
      {
        name: '--detail',
        type: 'boolean',
        required: false,
    ...
    output: {
      ...
      description:
        'JSON output contains baseline, repo, monitorId, entities, at, deltas, and summary fields. Text output contains an operator heartbeat and suggested actions.',
    },
