...
const OPTION_ENRICH = {
  name: '--enrich',
  valueName: 'review,comments,threads,body',
  type: 'string',
  required: false,
  grammar:
    'comma-separated unique values from: review, comments, threads, body; input order is canonicalized',
  description:
    "After a successful snapshot write only, fetch bodies for matching emitted deltas: review-changed (review), new-comments (comments), unresolved-threads-added (threads), or new/first-seen/reopened/baseline-state (body -- one nodes() call per delta for the item's own body, attached as enrichment.body = { body, mentions }; never called for a plain updated). Off by default; failures are warnings and never change detection state.",
};
...
const HELP_SPECS = {
  'gh-delta': {
    ...
    usage:
      'gh-delta [--repo <owner/name>] [--monitor-id <id>] [--state-file <path> | --state-dir <dir>] [--entities pr,issue] [--format json|text|compact|ndjson] [--summary-line] [--detail] [--summaries] [--enrich review,comments,threads,body] [--only-classes <classes>] [--ignore-classes <classes>] [--ignore-authors <logins>] [--settled] [--baseline-emit-state] [--log] [--outpost-url <url>] [--outpost-secret <ENV_VARIABLE_NAME>] [--outpost-timeout-ms <ms>] [--outpost-max-posts <n>] [--gh-timeout-ms <ms>] [--rate-limit-floor <n>] [--no-registry] [--lock-stale-ms <duration>]',
    purpose:
