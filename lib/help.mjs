...
const OPTION_ENRICH = {
  name: '--enrich',
  valueName: 'review,comments,threads',
  type: 'string',
  required: false,
  grammar:
    'comma-separated unique values from: review, comments, threads; input order is canonicalized',
  description:
    'After a successful snapshot write only, fetch bodies for matching emitted review-changed, new-comments, or unresolved-threads-added deltas. Off by default; failures are warnings and never change detection state.',
};
...
const HELP_SPECS = {
  'gh-delta': {
    ...
    usage:
      'gh-delta [--repo <owner/name>] [--monitor-id <id>] [--state-file <path> | --state-dir <dir>] [--entities pr,issue] [--format json|text|compact|ndjson] [--summary-line] [--detail] [--summaries] [--enrich review,comments,threads] [--only-classes <classes>] [--ignore-classes <classes>] [--ignore-authors <logins>] [--settled] [--baseline-emit-state] [--log] [--outpost-url <url>] [--outpost-secret <ENV_VARIABLE_NAME>] [--outpost-timeout-ms <ms>] [--outpost-max-posts <n>] [--gh-timeout-ms <ms>] [--rate-limit-floor <n>] [--no-registry] [--lock-stale-ms <duration>]',
    purpose:
  ...
  'gh-delta status': {
    ...
    options: [
      ...
      },
      {
        ...
        required: false,
        description:
          'Perform one normal detector tick before the local status read; this is the only status mode that contacts GitHub and writes state.',
      },
      OPTION_FORMAT,
      ...HELP_OPTIONS,
    ],
