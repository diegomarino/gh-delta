...
const HELP_SPECS = {
  'gh-delta': {
    ...
    usage:
      'gh-delta [--repo <owner/name>] [--monitor-id <id>] [--state-file <path> | --state-dir <dir>] [--entities pr,issue] [--format json|text] [--summary-line] [--detail] [--summaries] [--only-classes <classes>] [--ignore-classes <classes>] [--settled] [--baseline-emit-state] [--log] [--outpost-url <url>] [--outpost-timeout-ms <ms>] [--outpost-max-posts <n>] [--gh-timeout-ms <ms>] [--no-registry] [--lock-stale-ms <duration>]',
    purpose:
  ...
  'gh-delta read': {
    ...
    usage:
      'gh-delta read --cursor <path> [--only-classes <classes>] [--number <positive integer>] [--advance] [--format json|text]',
    purpose:
      'Read the cursor-bound log only. Never contacts GitHub, acquires a snapshot lock, or changes a snapshot; --advance atomically moves the cursor to the complete tail scanned.',
    options: [
      {
        name: '--cursor',
        valueName: 'path',
      ...
      },
      {
        ...
        required: false,
        description:
          'Atomically store the complete log tail scanned after assembling the report; rejected records are also advanced past.',
      },
      OPTION_FORMAT,
      ...HELP_OPTIONS,
    ],
  ...
  'gh-delta cursor set': {
    ...
    usage: 'gh-delta cursor set <cursor-path> <seq> [--log-file <path>] [--format json|text]',
    purpose:
      'Set a non-negative sequence for one cursor. A missing cursor requires --log-file; an existing cursor remains bound to its original log.',
    options: [
      {
        name: '--log-file',
        valueName: 'path',
