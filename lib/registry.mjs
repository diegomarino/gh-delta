...
export function defaultMachineId({ hostname = osHostname } = {}) {
  return `host-${createHash('sha1')
...
export function registerMonitor({
  ...
  try {
    ...
    // A missing or corrupt previous entry is safely replaced by this attempt.
  }
  const entry = {
    ...
    ...(typeof watchDir === 'string' ? { watchDir: resolve(watchDir) } : {}),
    ...(scope === 'watch-pr' ? { scope } : {}),
    machineId,
    lastRun: status === 'ok' ? at : (prior.lastRun ?? null),
    lastAttemptAt: at,
    lastOkAt: status === 'ok' ? at : (prior.lastOkAt ?? prior.lastRun ?? null),
    lastError:
      status === 'ok'
        ? null
        : { kind: error?.kind ?? 'io', message: String(error?.message ?? error ?? ''), at },
  };
...
function isValidEntry(entry) {
  return (
    ...
    (entry.watchDir === undefined || typeof entry.watchDir === 'string') &&
    (entry.scope === undefined || entry.scope === 'watch-pr') &&
    (typeof entry.lastRun === 'string' || entry.lastRun === null)
  );
}
