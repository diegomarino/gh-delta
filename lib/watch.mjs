 ...
 *
 * Bounded critical section -- but bounded by `leaseMs`, a caller-supplied
 * duration meaning ONLY "how long may this entry legitimately stay locked",
 * never by `--gh-timeout-ms`. Round 10 of this line of fixes: the first
 * version of this function accepted `{ghTimeoutMs, staleMs}` and the caller
 * passed the detector's OWN `--gh-timeout-ms`/`--lock-stale-ms` straight
 * through -- but `fn` here does a disk write (a mark write, then a full
 * snapshot serialize-and-rename), not a GitHub API call, and a user has no
 * reason to expect tuning their NETWORK timeout down to also shorten a
 * FILESYSTEM lock's lease. Set `--gh-timeout-ms` below the actual snapshot
 * write time and the lease expires mid-publish, a concurrent `watch add`
 * legitimately steals the "abandoned" lock, and the unmarked-replacement
 * race this whole mechanism exists to close returns -- silently, and for a
 * reason that has nothing to do with anything the operator was adjusting.
 *
 * `leaseMs` is `--lock-stale-ms` at the one call site (lib/cli.mjs's
 * run()), not a new flag: it is ALREADY the user-facing knob for "how long
 * is a lock legitimately held before something else may treat it as
 * abandoned", the exact question this lease answers, and its generous
 * 10-minute default swamps any realistic local disk write by orders of
 * magnitude without introducing a third duration concept for one caller to
 * reason about. Renewal (extending the lease mid-publish, the way the
 * state-file lock extends itself per completed GitHub fetch page) was
 * considered and rejected here: a lease this generous relative to a local
 * disk write does not need it, and renewal is exactly where a mistake
 ...
 * does not, and if the operator's `--lock-stale-ms` genuinely is not
 * generous enough for their disk, that is the same knob they would already
 * reach for to fix the identical problem on the state-file lock.
 *
 ...
 * the entry lock.
 */
export function withTerminalMarkLocks(paths, fn, { leaseMs }) {
  const held = [];
  try {
