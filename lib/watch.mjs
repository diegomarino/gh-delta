...
function filename(entry) {
  return `${entry.entity}-${entry.number}.json`;
}
...
export function addWatch(dir, raw, until, { now = () => new Date().toISOString() } = {}) {
  ...
  const path = join(dir, filename(item));
  return locked(path, () => {
    try {
      ...
      if (err.code !== 'ENOENT') throw new Error(`invalid watch entry ${filename(item)}`);
    }
    const entry = { ...item, until, addedAt: now() };
    atomic(path, entry);
...
export function removeWatch(dir, raw) {
  const item = parseWatchItem(raw);
  const path = join(dir, filename(item));
  return locked(path, () => {
    try {
