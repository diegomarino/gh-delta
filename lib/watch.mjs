 ...
 *
 * Bounded critical section, sized like the state-file lock's own timeout
 * rather than this module's usual short-lived {ghTimeoutMs: 1000}: `fn` is
 * expected to include a full snapshot publish, not just one small JSON
 * write, so the caller passes the SAME `ghTimeoutMs`/`staleMs` its
 * state-file lock already uses (see lib/cli.mjs's run()). This keeps the
 * two locks' generosity proportional to the same "how long can one unit of
 * real work take" budget the operator already controls via
 * --gh-timeout-ms/--lock-stale-ms, rather than inventing a second, smaller
 * ceiling a slow filesystem could blow through -- a lock that expires
 * WHILE still legitimately held would let a concurrent `watch add` steal
 * it mid-publish, reopening exactly the race this function exists to close.
 ...
 * older than `staleMs` (see lib/lock.mjs's acquireLock).
 */
export function withTerminalMarkLocks(paths, fn, { ghTimeoutMs, staleMs }) {
  const held = [];
  try {
    for (const path of [...new Set(paths)].sort()) {
      const acquired = acquireLock(path, { ghTimeoutMs, staleMs });
      if (!acquired.ok) {
