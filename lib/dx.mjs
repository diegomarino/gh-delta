...
export function initializeMonitor(
  { repo, stateDir, monitorId, entities = 'pr,issue', configPath },
  {
  ...
  return {
    ...
    report: {
      ...
      stateDir,
      configPath,
      stateFile: baseline.report?.stateFile,
      baseline: Boolean(baseline.report?.baseline),
...
export function runDoctorChecks(
  ...
  const checks = [
    ...
    row('node', nodeVersion() >= 18, `Node ${nodeVersion()} (requires >=18)`),
    row(
      'registry',
