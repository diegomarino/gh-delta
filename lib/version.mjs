 ...
 */
export function renderVersionText(
  metadata = getPackageMetadata(),
  channel = process.env.GH_DELTA_CHANNEL ?? 'npm',
) {
  const distribution = ['npm', 'gh extension', 'brew'].includes(channel) ? channel : 'npm';
  return `${metadata.name} ${metadata.version} (${distribution}) https://github.com/diegomarino/gh-delta/releases\n`;
}
