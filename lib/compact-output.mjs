...
// `context`/`changed`/`summary` are precomputed once by lib/cli/delta-details.mjs's
...
export function compactDelta(delta, { detail = false, full = false } = {}) {
  return {
    ...
    ...(delta.firstObserved !== undefined ? { firstObserved: delta.firstObserved } : {}),
    ...(delta.seq !== undefined ? { seq: delta.seq } : {}),
...
// Bare pre-flight error (no repo ever resolved -- see lib/cli/runner.mjs's run())
