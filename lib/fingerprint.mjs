 ...
 * digests (derived from the same fetch data), and `mergeStateStatus` is a
 * summary-only observation; dropping all three keeps change comparison and delta
 * ids stable across snapshots written before those fields existed. When the
 * fingerprint carries PR review-context fields, absent thread counts are
 * backfilled to zero so an older snapshot compares equal to a current one.
 */
