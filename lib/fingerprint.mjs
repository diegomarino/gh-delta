 ...
 * repo resolution end-to-end (lib/repo-source.mjs, lib/cli/detector.mjs, snapshot/
...
export function buildChecks(rows = []) {
  return sortRows(
    rows,
    // name:kind:status:conclusion alone is not unique: two matrix jobs, a
    // re-run, or a CheckRun and a StatusContext can share all four while
    // differing only in detailsUrl -- without it as a final tiebreaker the
    // stable sort just preserves GitHub's own (unstable) response order,
    // producing a phantom ci-changed / churned delta.id on a mere reorder.
    (row) => `${row.name}:${row.kind}:${row.status}:${row.conclusion}:${row.detailsUrl ?? ''}`,
  ).map((row) => {
...
// lib/cli/detector.mjs's strip step, right before a delta reaches the durable log,
