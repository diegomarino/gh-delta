...
function fsyncManifestDirectory(logFile, fs, deps) {
  const directory = dirname(publicationManifestPath(logFile));
  // libuv needs a writable directory handle for FlushFileBuffers on Windows;
  // POSIX uses a non-mutating read handle.
  const flags = (deps.platform ?? process.platform) === 'win32' ? 'r+' : 'r';
