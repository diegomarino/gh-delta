...
function validateManifest(value, path) {
  ...
  if (!(
    (value.version === 1 && keys === 'byteLength,lastSeq,version') ||
    (value.version === 2 && keys === 'byteLength,firstSeq,lastSeq,version') ||
    (value.version === 3 && keys === 'byteLength,dataFile,firstSeq,lastSeq,version')
  ))
    throw logError(`${path}: fields must match the version 1, 2, or 3 manifest contract`);
  if (![1, 2, 3].includes(value.version)) throw logError(`${path}: unknown version`);
  ...
  const firstSeq = value.version === 1 ? 1 : value.firstSeq;
  ...
  if (
    value.version === 3 &&
    (typeof value.dataFile !== 'string' || basename(value.dataFile) !== value.dataFile)
  )
...
function publishedLog(logFile, fs) {
  ...
  if (!manifest) {
    ...
    return {
      ...
      firstSeq: legacy.entries.length === 0 ? 1 : 1,
      lastSeq: legacy.entries.length,
      byteLength: legacy.completeBytes,
    };
  }
  const activeFile = dataPath(logFile, manifest);
  const size = logSize(activeFile, fs);
...
export function appendDeltaLog(logFile, { detectedAt, deltas }, deps = {}) {
  ...
  publishManifest(
    logFile,
    {
      ...(inspected.manifest?.version === 3
        ? {
            version: 3,
            firstSeq: inspected.manifest.firstSeq,
            dataFile: inspected.manifest.dataFile,
          }
        : inspected.manifest?.version === 2
          ? { version: 2, firstSeq: inspected.manifest.firstSeq }
          : { version: 1 }),
...
function readDeltaLogOnce(logFile, { afterSeq = 0, select = () => true } = {}, deps = {}) {
  ...
  let published;
  if (manifest) {
    published = publishedLog(logFile, fs);
  } else {
    const legacy = readLegacyLog(logFile, fs);
    // A legacy read that began while a writer bootstrapped its first manifest
    // must not expose the writer's post-boundary suffix.
    manifest = readManifest(logFile, fs);
    published = manifest
      ? publishedLog(logFile, fs)
      : {
          entries: legacy.entries,
          firstSeq: 1,
          lastSeq: legacy.entries.length,
