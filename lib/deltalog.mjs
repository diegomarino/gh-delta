...
// Best-effort dataFile recovery for resetDeltaLog only: a manifest that
// fails full validateManifest() (e.g. an extra/renamed field, schema drift)
// may still be parseable JSON naming a real, same-directory generation file.
// Reset's whole point is recovering from exactly this kind of corruption, so
// discarding the dataFile reference along with the invalid manifest would
// silently leave the generation file (and every historical delta in it) on
// disk while reporting success. Returns null whenever the JSON itself is
// unparseable (nothing to recover) or `dataFile` fails the same
// same-directory-basename check validateManifest enforces -- this must never
// be looser than that check, or reset could be made to unlink an arbitrary
// path via a crafted manifest.
function manifestDataFileHint(logFile, fs) {
  const path = publicationManifestPath(logFile);
  let raw;
  try {
    raw = fs.readFileSync(path, 'utf8');
  ...
  const candidate = isPlainObject(parsed) ? parsed.dataFile : undefined;
  return typeof candidate === 'string' && candidate && basename(candidate) === candidate
    ? candidate
    : null;
}
...
export function resetDeltaLog(logFile, deps = {}) {
  ...
  // A fully valid manifest (the common case) already names the right
  // dataFile via dataPath(). When the manifest itself failed validation,
  // still try to recover a safe same-directory dataFile reference from the
  // raw JSON (see manifestDataFileHint) rather than falling all the way back
  // to `logFile` -- reset exists precisely to clean up after manifest
  // corruption, so leaving a compacted generation file's historical deltas
  // on disk while reporting success is the worst failure direction here.
  const dataFile = manifest
    ? dataPath(logFile, manifest)
    : (() => {
        const hint = manifestDataFileHint(logFile, fs);
        return hint ? join(dirname(logFile), hint) : logFile;
      })();
  const removed = [];
  const missing = [];
  ...
  const manifestFile = publicationManifestPath(logFile);
  try {
    fs.unlinkSync(manifestFile);
    removed.push(manifestFile);
  } catch (error) {
    if (error?.code !== 'ENOENT') throw error;
    missing.push(manifestFile);
  }
