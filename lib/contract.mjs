...
// `firstObserved` (TODO(R6): populated by R6) is reserved and not yet set by
// any code. `seq` is populated by R4 (lib/cli.mjs, sourced from
// appendDeltaLog's {fromSeq, toSeq} return value): the delta's journal
// record number when the run used --log, absent otherwise.
...
// and never hashed into delta.id. `title` is null on the missing lifecycle
// (to === null); `headRefName` is PR-only.
