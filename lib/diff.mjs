...
// StatusContext sharing one name, or a matrix job re-run) AND the arrays
// genuinely differ: the name-keyed maps would silently collapse the
// duplicates and misreport the failed/fixed/changed breakdown, so the caller
// must fall back to { opaque: true } instead -- mirrors diffSummaries()
// below for the same reason. Byte-identical duplicate-name arrays are
// checked FIRST and short-circuit to "no change" (`{}`) before the
// duplicate-name ambiguity is even considered: fingerprint arrays are
// already deterministically sorted (see lib/fingerprint.mjs buildChecks), so
// an exact JSON match here means nothing about checks changed, regardless of
// duplicate names elsewhere in the same delta (e.g. a label-only update).
function checksDiff(from, to, limit) {
  const fromRows = Array.isArray(from) ? from : [];
  const toRows = Array.isArray(to) ? to : [];
  if (same(fromRows, toRows)) return {};
