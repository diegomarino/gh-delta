  ...
  return [...keys]
    .filter(
      (key) =>
        ...
        !['checks', 'reviews', 'threads'].includes(key),
    )
...
// Closes the TODO(v0.2) doctor/reset gap: re-baselining a monitor used to
// require parsing a `snapshot`/`log` error and deleting owned state by hand.
...
function waitSummaryMatches(report, condition, numbers) {
  ...
  return (report.deltas ?? []).some(
    (delta) =>
      ...
      condition.values.has(deltaSummary(delta)?.[condition.field]),
  );
}
...
async function runWait(argv, deps = {}) {
  ...
  const waited = await runBoundedWait({
    ...
    heartbeatFileFor: (tickReport) => {
      if (!heartbeatFile && tickReport.stateFile) heartbeatFile = `${tickReport.stateFile}.hb`;
    ...
    matches: (tickReport, iterations) => {
      ...
      const currentMatch =
        ...
        (!values['from-log'] &&
          (tickReport.stateFile
            ? snapshotSummaryMatches(
                readSnapshot(tickReport.stateFile),
                untilSummary.condition,
                numberSelection.numbers,
              )
            : derivedWaitStateFiles.some((stateFile) => {
                try {
  ...
  if (waited.reason === 'error') {
    const lastError = waited.errors.at(-1);
    ...
    return { ...failed, warnings: waited.warnings, progress: waited.progress };
  }
  const report = {
    schemaVersion: REPORT_SCHEMA_VERSION,
    command: 'wait',
...
function runSingle(argv, deps = {}) {
  ...
  const failedAttempt = (kind, error, extra = {}) => {
    ...
    return {
      ...errorResult(kind, String(error?.message ?? error), { ...ids, ...extra }, format),
  ...
  try {
    ...
    // down. TODO(R3): surface this as `results[].rateLimit` once the v2
    // report envelope exists; for now it is only threaded this far.
    ...
    let baseline, deltas, snapshot, filteredDeltas, rawDeltas;
    ...
    try {
      ...
      if (attentionFiltering) {
        ...
        for (const d of deltas)
          enrichDelta(d, {
            summaries: values.summaries || values.settled || ['compact', 'ndjson'].includes(format),
          });
        ...
        if (ignoreAuthors.authors.length) {
          ...
          for (const delta of deltas) {
            ...
            if (!increments.length) {
              // review-comments-added fired (reviewComments rose in aggregate) but
              // threadReplyIncrements found nothing to name -- every thread behind
              // the rise is brand new this tick (no `from` baseline to diff replies
              // against, see lib/detect.mjs's threadReplyIncrements doc comment).
              // This is exactly as unverifiable as the !doubleOptIn case above and
              // MUST warn the same way: silently continuing here would let a
              // brand-new bot-authored thread's replies through unfiltered even
              // with both opt-in flags set, which is worse than the single-flag
              // case (that one at least always warns).
              ignoreAuthorsWarnings.push({
                label: 'ignore-authors thread-replies',
                reason: `delta ${delta.id} (${delta.entity} #${delta.number}) carries review-comments-added entirely from thread(s) with no prior baseline to diff; cannot verify reply authors, failing open`,
              });
              continue;
            }
            try {
        ...
        for (const d of deltas) {
          enrichDelta(d, {
            ...
            legacyLine: values.detail || format === 'text',
            details: values.detail,
            summaries: values.summaries || ['compact', 'ndjson'].includes(format),
          });
        }
      }
      if (selectedNumbers)
    ...
    // TODO(R3): surface as results[].rateLimit alongside tickRateLimit above.
    ...
    return {
      ...
      // quota spend (observation fetches above + enrichment above). TODO(R3):
      // this is the value to surface as `results[].rateLimit` once the v2
      // report envelope exists; sibling to `report` deliberately, so building
      // that envelope stays entirely R3's decision.
...
export function run(argv, deps = {}) {
  ...
  if (selected.repos.length === 0) return runSingle(argv, deps);
  // One normalized repository intentionally follows the untouched one-repo
  // path.  This also makes repeated/case-varied spellings harmless aliases.
  const oneArgv = [...selected.rest, '--repo', selected.repos[0]];
  if (selected.repos.length === 1) return runSingle(oneArgv, deps);
  ...
  const results = [];
  for (const [index, repo] of selected.repos.entries()) {
    results.push({
      repo,
  ...
  for (const { repo, result } of results) {
    if (result.report?.error) {
      errors.push({
        ...
        hint: result.report.hint,
        ...(result.report.resetAt ? { resetAt: result.report.resetAt } : {}),
      });
    } else {
      deltas.push(...(result.report?.deltas ?? []).map((delta) => ({ ...delta, repo })));
      if (result.report && Object.hasOwn(result.report, 'filteredDeltas')) {
        hasFilter = true;
        filteredDeltas += result.report.filteredDeltas;
      }
    }
    warnings.push(
      ...(result.warnings ?? []).map((warning) => ({
        ...warning,
        label: `${repo}: ${warning.label}`,
      })),
    );
  }
  const code = errors.some(
    (error) => results.find((row) => row.repo === error.repo)?.result.code === 2,
  )
    ? 2
    : errors.length
      ? 1
  ...
  const report = {
    ...
    entities: parseEntitySelection(parsed.values.entities).selected,
    at,
    repos: selected.repos,
    deltas,
    ...(hasFilter ? { filteredDeltas } : {}),
    errors,
    summary: `${deltas.length} delta(s) across ${selected.repos.length} repo(s); ${errors.length} error(s)`,
  };
  return { code, report, format: parsed.format, warnings };
}
