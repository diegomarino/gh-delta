...
import { closeSync, mkdirSync, openSync, statSync, readFileSync, utimesSync } from 'node:fs';
...
function runStatus(argv, deps = {}) {
  ...
  if (values.refresh) {
    ...
    return runStatus(refreshArgs, deps);
  }
  if (values['state-file'] && values['state-dir'])
    return errorResult(
      'config',
...
export function run(argv, deps = {}) {
  ...
  if (['watch', 'log', 'list', 'read', 'cursor', 'schema', 'status'].includes(argv[0]))
...
export async function runCommand(argv, deps = {}) {
  const result =
    argv[0] === 'wait' ? await runWait(argv.slice(1), deps) : await runWithOutpost(argv, deps);
  const now = deps.now ?? (() => new Date().toISOString());
