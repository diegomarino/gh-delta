...
const ERROR_EXIT_CODES = { config: 2, snapshot: 2, github: 1, io: 1, busy: 1 };
...
export function run(argv, deps = {}) {
  ...
  // fetch. numberOfFetches bounds the lease: at most one fetchPRs and one
  // fetchIssues call happen below, per --entities.
  const numberOfFetches = (entitySelection.wantsPr ? 1 : 0) + (entitySelection.wantsIssue ? 1 : 0);
  ...
  try {
    ...
    if (!assertLockOwned(stateFile, lockToken, lockDeps)) {
      return errorResult('busy', `lock lost before snapshot write: ${stateFile}`, ids, format);
    }
    ...
    return {
      ...
      report,
      format,
      warnings: lockWarning ? [...derivationWarnings, lockWarning] : derivationWarnings,
    };
  } finally {
