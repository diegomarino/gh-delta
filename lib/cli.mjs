  ...
  const items = entities.selected
    .flatMap((entity) =>
      Object.entries(snapshot?.[entity] ?? {})
        .filter(
          ([number, fp]) =>
            fp.state === 'OPEN' && (!numbers.numbers || numbers.numbers.has(Number(number))),
        )
        .map(([number, fp]) => ({
          entity,
          number: Number(number),
          summary: entity === 'pr' ? deltaSummary({ entity, to: fp }) : null,
          lastChangedAt: fp.lastChangedAt ?? null,
          ticksSinceChange: fp.ticksSinceChange ?? 0,
        })),
    )
...
function runInit(argv, deps = {}) {
  ...
  const result = initializeMonitor(
    { repo: repo.repo, stateDir, monitorId, entities: entityValue, configPath },
    {
...
function runDoctor(argv, deps = {}) {
  ...
  const result = runDoctorChecks(
    { repo: repo.repo, stateDir, monitorId, machineId: deps.machineId ?? defaultMachineId() },
    {
      ...
      ghAuthenticated: () =>
        safeGh(['auth', 'status', '--active', '--hostname', 'github.com', '--json', 'hosts']),
...
function snapshotSummaryMatches(snapshot, condition, numbers) {
  ...
  return Object.entries(snapshot.pr ?? {}).some(
    ([number, fingerprint]) =>
      ...
      condition.values.has(deltaSummary({ entity: 'pr', to: fingerprint })?.[condition.field]),
  );
}
...
function runSingle(argv, deps = {}) {
  ...
  try {
    ...
    try {
      ...
      // The durable log deliberately sees the pre-enrichment public stream.
      // Keep its private-identity twin only until snapshot publication, when
      // opt-in decoration is allowed to spend GitHub quota.
      rawDeltas = deltas;
      deltas = rawDeltas.map(hideInternalDetails);
    } catch (err) {
      return failedAttempt('github', err);
    }
    ...
    const enrichmentWarnings = enrichmentSelection.kinds.length
      ...
      : [];
    if (enrichmentSelection.kinds.length) deltas = rawDeltas.map(hideInternalDetails);
