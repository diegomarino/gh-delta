...
// Consumer-side replay: no GitHub, snapshot, lock, registry, or cursor write
// unless --advance is explicitly requested.
  ...
  try {
    ...
    try {
      ...
      if (attentionFiltering) {
        ...
        ({ deltas, filteredDeltas } = applyAttentionFilters(deltas, {
    ...
    if (values.log && deltas.length > 0) {
      try {
        ...
        if (err?.code === 'LOCK_LOST') {
          return errorResult('busy', `lock lost before delta log write: ${stateFile}`, ids, format);
        }
        return errorResult(
          err?.kind === 'log' ? 'log' : 'io',
          String(err?.message ?? err),
