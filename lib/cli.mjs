...
function runSingle(argv, deps = {}) {
  ...
  try {
    ...
    for (const delta of deltas) {
      ...
      // `merged` CLASS. Requiring the matching terminal class to have
      // survived filtering is what lets `--ignore-classes merged` actually
      // protect a `--until merged` entry: classifyPr/classifyIssue fire
      // `merged` only for a `state === 'merged'` transition and `closed`
      // only for `state === 'closed'`, so this is never a stricter check on
      // the unfiltered case, only a check that the filter didn't erase it.
      const terminalClass = state === 'merged' ? 'merged' : state === 'closed' ? 'closed' : null;
      const hasSurvivingTerminalClass =
        terminalClass !== null && delta.classes.includes(terminalClass);
      if (
        watched &&
        hasSurvivingTerminalClass &&
