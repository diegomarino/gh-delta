...
function parseWaitSummary(raw) {
  ...
  const selected =
    values
      ...
      .filter(Boolean) ?? [];
  if (!field || !/^[A-Za-z][A-Za-z0-9]*$/.test(field) || selected.length === 0)
...
async function runWait(argv, deps = {}) {
  ...
  const format = values.format;
  if (!['json', 'text', 'compact', 'ndjson'].includes(format)) {
    const result = waitConfigError(argv, deps, '--format must be json, text, compact, or ndjson');
    return { ...result, warnings: [], progress: '' };
  }
  if (!values.timeout) {
      ...
      const result = values['from-log']
        ? run(
            [
              ...
              ...(values.until ? ['--only-classes', values.until] : []),
            ],
            deps,
          )
        : run(tickArgv, deps);
...
export function run(argv, deps = {}) {
  ...
  if (['watch', 'log', 'list', 'read', 'cursor', 'schema'].includes(argv[0]))
