    ...
    try {
      if (watchTerminalIgnoresToRecord.length) {
        ...
        withWatchTerminalMarkLocks(
          ...
          // `leaseMs` deliberately reuses --lock-stale-ms, never
          // --gh-timeout-ms: this critical section is a disk write (the mark,
          // then the snapshot publish), not a GitHub call, and has nothing to
          // do with the network timeout an operator tunes independently. See
          // withTerminalMarkLocks's doc comment (round 10 of this finding).
          { leaseMs: lockStaleMs.ms },
        );
      } else {
        publishSnapshot();
      }
