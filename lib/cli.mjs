...
function runSingle(argv, deps = {}) {
  ...
  try {
    ...
    if (watchTerminalIgnoresToRecord.length) {
      // Same ordering rule as the durable log immediately above, and for the
      // identical reason: `ignoredTerminalAt` is REQUIRED state, not
      // best-effort cleanup like the watch-entry REMOVAL in the loop below
      // (a failed removal just delays cleanup to a later, idempotent retry
      // -- no bug results). A filtered terminal transition, by contrast, is
      // unrepeatable: the very next tick's snapshot already records the
      // terminal state, so if this marker failed to persist and the tick
      // proceeded to publish anyway, a later metadata-only delta under the
      // SAME filter would see no `ignoredTerminalAt` and silently remove
      // the watch entry -- reproducing exactly the premature-cleanup bug
      // this marker exists to prevent, permanently (the transition can
      // never be observed again once the snapshot advances). Do not move
      // this after snapshot publication.
      //
      // Failing the whole tick here is deliberately NOT disproportionate:
      // the failure modes are the same class as the ones that already fail
      // the tick a few lines up (an unwritable state directory, a lost
      // lock) -- an unwritable watch directory is no less "this monitor
      // cannot make progress right now" than an unwritable state file.
      //
      // The per-watch-entry lock (lib/watch.mjs's `locked()`, keyed on the
      // entry's OWN path) is a genuinely SEPARATE resource from the
      // state-file lock this tick holds throughout (`lockToken`) -- unlike
      // `reset`, which acquires that SAME state-file lock and releases it
      // last specifically so a concurrent tick serializes against it (see
      // runReset's own comment), `watch add`/`rm` never touch the
      // state-file lock at all. A concurrent `watch add` replacing this
      // exact entry is therefore genuinely possible, not merely
      // theoretical, and a benign-looking `false` return here is real
      // information, not noise: either the bytes were already stale when
      // we tried to mark (caught below), or -- because `addWatch` always
      // builds a brand-new entry object when replacing, never preserving
      // `ignoredTerminalAt` from the entry it replaces -- a race lands
      // AFTER a successful mark and silently wipes it before publication
      // (caught by the re-verify pass immediately below, mirroring
      // writeSnapshotAtomic's own verifyBeforeCommit fence for the exact
      // same reason: shrink the race to the smallest possible window
      // rather than pretend two independent locks make it impossible).
      // Retrying the mark itself is deliberately NOT done here: retrying
      // around a lock this tick does not own is how these turn into
      // livelocks against an operator's own `watch add`, not a fix.
      try {
        for (const watched of watchTerminalIgnoresToRecord) {
          if (!markWatchTerminalIgnored(watched.path, watched.bytes, at)) {
            return failedAttempt(
              'busy',
      ...
      // Re-verify every mark just made is STILL there, immediately before
      // publication: a concurrent replacement landing in the gap between a
      // successful mark above and the snapshot rename below would
      // otherwise publish the terminal state with an unmarked entry, the
      // exact bug this whole mechanism exists to prevent.
      for (const watched of watchTerminalIgnoresToRecord) {
        let stillMarked;
        try {
          stillMarked =
            JSON.parse(readFileSync(watched.path, 'utf8')).ignoredTerminalAt !== undefined;
        } catch {
          stillMarked = false;
        }
        if (!stillMarked) {
          return failedAttempt(
            'busy',
            `watch entry was replaced after being marked, before publication: ${watched.path}`,
          );
        }
      }
    }
