...
function pushThreadSetDelta(details, klass, delta) {
  const { addedIds, removedIds } = threadSetDiff(
    fpOf(delta.from)?.threadStates,
    fpOf(delta.to)?.threadStates,
  );
  ...
  return [...keys]
    .filter(
      (key) =>
        ...
        // of them here would only duplicate the digest transition -- their
        // meaningful expression is the dedicated detail each drives instead
        // (ci-changed/review-changed's named breakdown, unresolved-threads-
        // added/-resolved's threadStates row).
        !['ciChecks', 'reviewSummary', 'threadDigest', 'threadStates'].includes(key),
    )
...
function summaryDiffExtra(delta, summaryField, key) {
  const fromFp = fpOf(delta.from);
  const toFp = fpOf(delta.to);
  const from = fromFp?.[summaryField];
  const to = toFp?.[summaryField];
  ...
  const withDetail = (row) => {
    const source = (toFp?.[detailField] ?? []).find(
      (candidate) =>
...
function errorHint(kind, error) {
  ...
  if (kind === 'snapshot')
    return 'Inspect the state file; restore valid JSON or choose a new durable --state-dir.';
  if (kind === 'log')
    return 'Inspect the durable delta log and cursor before retrying; do not truncate it automatically.';
  if (kind === 'io')
...
function commentAuthorsIgnored(delta, ignoredAuthors) {
  ...
  const fromFp = fpOf(delta.from);
  const toFp = fpOf(delta.to);
  const increment = (toFp?.comments ?? 0) - (fromFp?.comments ?? 0);
  const conversationIncrement =
    (toFp?.conversationComments ?? NaN) - (fromFp?.conversationComments ?? NaN);
  const rows = toFp?.commentNodes;
  if (
    increment <= 0 ||
...
function runSingle(argv, deps = {}) {
  ...
  try {
    ...
    try {
      ...
      // identities (reviewDetails/commentNodes/...) it needs to fetch bodies.
      rawDeltas = deltas;
    } catch (err) {
      return failedAttempt('github', err);
    }
    ...
    if (values.log && deltas.length > 0) {
      try {
        ...
        appendDeltaLog(
          ...
          { detectedAt: at, deltas: deltas.map((delta) => ({ ...delta })) },
          {
    ...
    const enrichmentWarnings = enrichmentSelection.kinds.length
