...
function pushNumericDelta(details, klass, delta, field) {
  ...
  if (klass === 'new-comments') {
    ...
    if (Array.isArray(added) && extra.delta > 0 && extra.delta <= added.length) {
...
function commentAuthorsIgnored(delta, ignoredAuthors) {
  ...
  if (increment <= 0 || !Array.isArray(rows) || increment > rows.length)
    return { delta, filtered: false };
  ...
  try {
    ...
    try {
      ...
      if (ignoreAuthors.authors.length) {
        ...
        filteredDeltas = authorFiltered.filter(({ filtered }) => filtered).length;
      }
      if (attentionFiltering) {
        ...
        filteredDeltas = (filteredDeltas ?? 0) + attention.filteredDeltas;
