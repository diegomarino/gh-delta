...
export function run(argv, deps = {}) {
  ...
  if (!values['state-file'] && !values['state-dir'])
    return errorResult(
      'config',
      'provide either --state-file <path> or --state-dir <dir>',
      { repo, monitorId: values['monitor-id'], at },
      format,
    );
  ...
  const stateFile =
    values['state-file'] ??
    snapshotPath(repo, values['monitor-id'], entitySelection.key, values['state-dir']);
  let old;
  try {
  ...
  let current;
  try {
    // Task 9 replaces this with snapshot-derived horizon.
    const horizonCutoff = null;
    // Fetch broadly; filtering too early would make close/merge/relabel events disappear.
    current = {
      pr: entitySelection.wantsPr
        ? fetchPRs(repo, { timeoutMs: ghTimeoutMs.value, horizonCutoff })
        : undefined,
      issue: entitySelection.wantsIssue
        ? fetchIssues(repo, { timeoutMs: ghTimeoutMs.value, horizonCutoff })
        : undefined,
    };
  } catch (err) {
