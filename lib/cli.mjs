...
/**
 * Run the read-only `list` subcommand: inventory the derived monitor snapshots
 * in one state directory. Never contacts GitHub and never writes state, so the
 * only failure kinds are `config` (bad flags) and `io` (unreadable directory).
 *
 * @param {string[]} argv - Arguments after the `list` token.
 */
function runList(argv, deps = {}) {
  const { listMonitors = fsListMonitors, now = () => new Date().toISOString() } = deps;
  const at = now();
  ...
  try {
    ({ monitors, skippedFiles } = listMonitors(stateDir, { sinceMs, now: () => at }));
  } catch (err) {
