...
function runSingle(argv, deps = {}) {
  ...
  try {
    ...
    for (const delta of deltas) {
      ...
      // must still recognize `merged` as terminal, per #57). But `state` is
      // read off the unfiltered fingerprint, so a delta surviving attention
      // filtering with an unrelated class (e.g. `relabeled`) still carries
      // its true `to.state` even after `--ignore-classes merged` strips the
      // `merged` CLASS -- requiring that TRANSITION class to have survived
      // filtering is what lets `--ignore-classes merged` actually protect a
      // `--until merged` entry, since classifyPr/classifyIssue only ever
      // attach `merged`/`closed` when a real state transition was observed
      // (their `if (oldFp.state !== fp.state)` guard).
      //
      // `firstObserved` (the `new`/`first-seen`/`baseline-state` classes) is
      // the other side of this: an OBSERVATION, not a transition. diffEntity
      // never combines `first-seen` with `merged`/`closed` (it is always a
      // bare one-element `classes` array -- see lib/detect.mjs), so a
      // watched item absent from the snapshot but already terminal on its
      // first observation (e.g. it merges between `watch add` and the first
      // poll) can NEVER carry a surviving transition class, filtered or not.
      // There is no prior transition tick for a filter to have meant "ignore
      // this" about, so requiring one here would strand the entry forever:
      // the snapshot records the terminal fingerprint, so no later tick ever
      // fires a delta for it again. Treat a terminal first observation as
      // always cleanup-eligible instead.
      const terminalClass = state === 'merged' ? 'merged' : state === 'closed' ? 'closed' : null;
      const cleanupEligible =
        terminalClass !== null &&
        (delta.firstObserved === true || delta.classes.includes(terminalClass));
      if (
        watched &&
        cleanupEligible &&
