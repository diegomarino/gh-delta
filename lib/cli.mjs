...
/**
 * Decide whether a delta's terminal `to.state` may trigger `--until`
 * watch-directory cleanup, distinguishing three cases that took four rounds
 * to separate cleanly:
 *
 *   1. First observation (`delta.from` is null: the `new`/`first-seen`/
 *      `baseline-state` classes -- see lib/detect.mjs's diffEntity, which
 *      never combines any of them with `merged`/`closed`). There is no
 *      prior tick's transition for an attention filter to have meant
 *      "ignore this" about -- the item was simply already terminal (or
 *      not) the very first time we ever saw it. Always eligible.
 *   2. No transition THIS tick (`delta.from.state` was ALREADY terminal).
 *      A same-state metadata-only delta (e.g. a broad-poll `relabeled`
 *      change on an item that merged before the watch was even added)
 *      carries no `merged`/`closed` class, because none was produced --
 *      the state did not just become terminal now. There is nothing this
 *      tick's filter could mean to protect, since no terminal transition
 *      happened this tick. Always eligible.
 *   3. A genuine transition this tick (`delta.from.state` was open,
 *      `to.state` is now terminal). classifyPr/classifyIssue attach the
 *      matching `merged`/`closed` class if and only if this happens (their
 *      `if (oldFp.state !== fp.state)` guard), so requiring that class to
 *      have survived attention filtering is exactly "did the operator ask
 *      to ignore this transition" -- the one case a filter must be able to
 *      protect the entry from.
 */
function isTerminalCleanupEligible(delta, terminalClass) {
  const priorState = delta.from?.state ?? null;
  if (priorState === null || TERMINAL_STATES.has(priorState)) return true;
  return delta.classes.includes(terminalClass);
}
...
function runSingle(argv, deps = {}) {
  ...
  try {
    ...
    for (const delta of deltas) {
      ...
      // `isTerminalCleanupEligible` decides whether THIS tick's delta may
      // act on that -- see its doc comment for the three-way distinction.
      const terminalClass = state === 'merged' ? 'merged' : state === 'closed' ? 'closed' : null;
      const cleanupEligible =
        terminalClass !== null && isTerminalCleanupEligible(delta, terminalClass);
      if (
        watched &&
        cleanupEligible &&
