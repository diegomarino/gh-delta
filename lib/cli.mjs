...
function runSingle(argv, deps = {}) {
  ...
  try {
    ...
    if (watchTerminalIgnoresToRecord.length) {
      ...
      // cannot make progress right now" than an unwritable state file. A
      // benign `false` return (already marked, or a concurrent replacement
      // -- see markTerminalIgnored's own doc comment) is not an error and
      // does not fail anything, matching removeWatchedFile's return value
      // being similarly unchecked below.
      try {
