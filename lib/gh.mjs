...
/**
 * Convert paginated GraphQL review-thread pages into per-PR unresolved/resolved
 * thread counts.
 *
 * The response is intentionally strict: if any open PR has more than one page of
 * review threads, the fetch fails so callers can avoid emitting potentially
 * incomplete `unresolved-threads-*` deltas.
 */
...
/**
 * Fetch open PR review-thread counters using GraphQL pagination.
 *
 * @param {string} repo - owner/name repository identifier
 * @param {(cmd: string, args: string[]) => string} [exec=defaultExec] - command executor
 * @returns {Map<number, {reviewThreads:number, unresolvedReviewThreads:number}>}
 * @throws when GraphQL response shape is unexpected or pagination is incomplete
 */
