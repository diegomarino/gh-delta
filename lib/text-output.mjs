    ...
    return [
      ...
      ...(report.errors?.length
        ? [
            '',
            'Partial errors:',
            ...report.errors.map((error) => `${error.repo} | ${error.kind}: ${error.message}`),
          ]
        : []),
...
export function formatWatchTextOutput({ report }) {
  if (report.error) return `${report.at} | watch error\n\n${report.error}`;
...
export function formatStatusTextOutput({ report, now }) {
  ...
  return [
    ...
    ...report.items.map((item) =>
      [
        `${item.entity.toUpperCase()} #${item.number}`,
...
export function formatCompactTextOutput({ report, now }) {
  const at = report.at ?? now();
  if (report.error)
    return `${at} | log compact error\n\ngh-delta log compact error: ${report.error}`;
  return [
