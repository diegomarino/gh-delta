...
/**
 * Render the human-readable output for a successful `gh-delta list` report.
 * Error reports go through `formatTextOutput` like every other command result.
 */
export function formatListTextOutput({ report }) {
  const heartbeat = `${report.at} | ${report.monitors.length} monitor(s) | ${report.stateDir}`;
