...
/**
 * Render warnings — derivation (plain strings) and outpost delivery
 * (`{ label, reason }`) alike — into text-safe log lines.
 */
  ...
  return [
    ...
    ...warnings.map((warning) =>
      typeof warning === 'string'
        ? warning
        : `outpost warning: ${warning.label} failed: ${warning.reason}`,
    ),
  ].join('\n');
}
