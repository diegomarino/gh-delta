...
/**
 * Normalize GitHub's `mergeStateStatus` to a lowercase enum.
 *
 * BEHIND | BLOCKED | CLEAN | DIRTY | DRAFT | HAS_HOOKS | UNSTABLE map directly;
 * UNKNOWN, null, absent, or any unrecognized value becomes `'unknown'`. Fail-closed
 * on purpose: a PR can be `mergeable` yet BEHIND its base (repos that require the
 * branch be up to date) or BLOCKED by an unsatisfied protection rule, so a consumer
 * treats `'unknown'` as "not computed", exactly like `mergeable: unknown`, rather
 * than reading a merge-readiness verdict that was never observed.
 *
 * @param {string|null|undefined} mergeStateStatus - raw `to.mergeStateStatus`
 * @returns {'behind'|'blocked'|'clean'|'dirty'|'draft'|'has_hooks'|'unstable'|'unknown'}
 */
export function normalizeMergeStateStatus(mergeStateStatus) {
  return MERGE_STATE_STATUSES[upper(mergeStateStatus)] ?? 'unknown';
}
...
export function prSummary(to) {
  ...
  return {
    ...
    mergeStateStatus: normalizeMergeStateStatus(to.mergeStateStatus),
 ...
 * (no `to`) get null. `to` is a snapshot item (`{ fingerprint, context, meta }`);
 * only `fingerprint` -- the compared fields the digests were hashed from -- feeds
 * the summary.
 *
...
export function deltaSummary(delta) {
  ...
  return prSummary(delta.to.fingerprint);
}
