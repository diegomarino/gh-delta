...
function changedReviews(delta) {
  ...
  if (!delta.classes.includes('review-changed') || !Array.isArray(to?.reviewDetails)) return null;
  const old = new Map(
    (Array.isArray(from?.reviewDetails) ? from.reviewDetails : [])
      .filter((row) => row?.id)
      .map((row) => [row.id, JSON.stringify(row)]),
  );
  const ids = to.reviewDetails
    .filter(
      (row) =>
...
function addedThreads(delta) {
  ...
  const { addedIds } = threadSetDiff(
    delta.from?.fingerprint?.threadStates,
    delta.to?.fingerprint?.threadStates,
  );
