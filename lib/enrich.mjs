...
import { threadSetDiff, threadReplyIncrements } from './detect.mjs';
...
function addedThreads(delta) {
  ...
  const { addedIds } = threadSetDiff(delta.from?.threads, delta.to?.threads);
  return addedIds.length ? addedIds : null;
}

function threadReplies(delta) {
  if (!delta.classes.includes('review-comments-added')) return null;
  const increments = threadReplyIncrements(delta.from?.threads, delta.to?.threads);
  return increments.length ? increments : null;
}

// `body` enrichment fetches the item's own body, not a child object's, so its
// identity is the item's GraphQL node id -- delta.context.id, the delta's own
// top-level identity field (never delta.to, which is the bare fingerprint and
// carries no context) -- available only for classes that carry a freshly
// observed object. Explicitly excludes plain `updated` (F2 scope boundary:
...
const REQUIRED_CLASSES = {
  ...
  body: BODY_CLASSES,
  'thread-replies': ['review-comments-added'],
};
