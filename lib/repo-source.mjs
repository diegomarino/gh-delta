...
const defaultExec = (cmd, args, { timeoutMs } = {}) =>
  ...
  execFileSync(cmd, args, { encoding: 'utf8', timeout: timeoutMs, stdio: ['ignore', 'pipe', 'pipe'] });
...
function ghRepoView(exec, ghTimeoutMs) {
  ...
  try {
    ...
    // A timeout is transient (retryable); any other non-zero is "not a GitHub
    // repo here" (permanent decline).
