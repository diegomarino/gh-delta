 ...
 *
 * Deliberately excludes the detection timestamp: the id is a stable identity for
 * "this repo/monitor/entity reached this class set", so a receiver can collapse
 * the same logical event seen across ticks or retries into one.
 */
 ...
 * while still reproducible. Receivers should dedupe semantic work by eventId;
 * deliveryId is for correlating one send attempt.
 */
...
export function buildOutpostPayload({ report, delta }) {
  ...
  return {
    ...
    // Content-addressed delta id (stable across runs and monitors). Distinct from
    // the delivery-envelope eventId/deliveryId below, which include monitorId.
    // The CLI stamps `delta.id` at report assembly; recompute here as a fallback
    // so the documented detectDeltas -> buildOutpostPayload embedding path (which
    // never runs the CLI report step) still emits a real id, not null.
    id: delta.id ?? deltaId(deltaIdentity(report.repo, delta)),
