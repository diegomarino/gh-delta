...
/**
 * Normalize a delta into the public schema v2 shape before it is embedded in
 * an outpost payload.
 *
 * The CLI's own delivery path (runWithOutpost) already stamps `id`/`repo`
 * and strips `from`/`to` down to the bare fingerprint before calling
 * buildOutpostPayload, so this is a no-op there. But `buildOutpostPayload`
 * is also documented as directly embeddable after a bare `detectDeltas`
 * call (see its fallback `id` computation above), and that path's deltas
 * still carry detectDeltas' raw shape: `from`/`to` as the full
 * `{fingerprint, context, meta}` snapshot item, and no `repo`/`summary`/
 * `changed` -- 3 of the 8 DELTA_CORE_REQUIRED fields missing entirely.
 * Every step here is idempotent against an already-normalized (CLI-shaped)
 * delta, so this can run unconditionally on both paths.
 */
function normalizeOutpostDelta(delta, fallbackRepo) {
  const repo = delta.repo ?? fallbackRepo;
  ...
  return {
    ...
    changed: delta.changed ?? diffFingerprint(delta.from?.fingerprint, delta.to?.fingerprint),
    from: delta.from?.fingerprint ?? delta.from ?? null,
    to: delta.to?.fingerprint ?? delta.to ?? null,
  };
}
...
export function buildOutpostPayload({ report, delta }) {
  ...
  const repo = delta.repo ?? report.repo;
  const normalized = normalizeOutpostDelta(delta, repo);
  return {
    type: 'gh-delta.delta',
    schemaVersion: OUTPOST_SCHEMA_VERSION,
    deliveryId: deliveryId(report, normalized, detectedAt),
