...
function eventId(report, delta, detectedAt) {
  // Sort classes so the id is order-independent: the same semantic delta yields
  // the same id regardless of the order the classifier happened to emit classes.
  const classes = [...(delta.classes ?? [])].sort().join('+');
  return [
    'gh-delta.delta.v1',
    report.repo,
    report.monitorId,
