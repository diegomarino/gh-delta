...
export function buildOutpostPayload({ report, delta }) {
  ...
  const toFp = to?.fingerprint ?? null;
  const fromFp = from?.fingerprint ?? null;
  return {
    type: 'gh-delta.delta',
    ...
    entity: delta.entity,
    number: delta.number,
    title: delta.context?.title ?? null,
    ...
    ...(delta.entity === 'pr' && delta.to != null
      ? { headRefName: delta.context?.headRefName ?? null }
      : {}),
    ...
    state: toFp?.state ?? fromFp?.state ?? null,
    labels: toFp?.labels ?? fromFp?.labels ?? [],
    line:
      delta.summaryLine ??
...
export async function sendOutposts({
  ...
  for (const [index, delta] of deltas.entries()) {
    ...
    try {
      ...
      warnings.push({
        label: `${aggregate ? `${payload.repo}: ` : ''}${payload.entity.toUpperCase()} #${payload.number}`,
