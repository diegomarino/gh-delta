...
// `firstObserved` (TODO(R6)) is reserved and not yet populated. `seq` is
// populated by R4: the delta's journal record number when the run used
// --log, present only then (never null -- absence IS the "no log" signal at
// the report level; the outpost payload's own top-level `seq` is the one
// that carries null explicitly). The one deliberate per-format property divergence is the
