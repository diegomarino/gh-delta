...
function entityKey(entities) {
  ...
  return slug(known.length ? known.join('-') : tokens.join('-'));
}

/** Build a filesystem-safe snapshot filename scoped by repository, monitor, and entity set. */
export function snapshotPath(repo, monitorId, entities, baseDir) {
  return `${baseDir}/${slug(repo)}__${slug(monitorId)}__${entityKey(entities)}.json`;
}
