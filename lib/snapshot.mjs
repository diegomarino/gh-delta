 ...
 * numeric string keys to objects, with an optional object `meta`.
 *
...
function validateSnapshotShape(snapshot, path) {
  ...
  for (const family of ['pr', 'issue']) {
    for (const [key, value] of Object.entries(snapshot[family])) {
      ...
      if (Object.hasOwn(value, 'lastChangedAt') && !isValidDateString(value.lastChangedAt)) {
        throw new Error(
          `invalid snapshot shape at ${path}: ${family}.${key}.lastChangedAt must be an ISO date string when present`,
        );
      }
    }
  }
  ...
  if (!horizon) {
    for (const family of ['pr', 'issue']) {
      for (const fp of Object.values(oldSnapshot[family] ?? {})) {
        if (typeof fp?.updatedAt !== 'string') continue;
        if (!isValidDateString(fp.updatedAt)) {
          throw new Error(`invalid snapshot horizon in ${family} fingerprint updatedAt`);
        }
        if (!horizon || fp.updatedAt > horizon) {
