 ...
 * anything the reader would later reject. Requires `{ pr, issue }` maps of
 * numeric string keys to three-section items (`fingerprint`/`context`/`meta`),
 * with an optional object top-level `meta` (the snapshot-wide horizon).
 *
...
function validateSnapshotShape(snapshot, path) {
  ...
  if (
    ...
    Object.hasOwn(snapshot.meta, 'horizon') &&
    !isValidDateString(snapshot.meta.horizon)
  ) {
    throw new Error(
      `invalid snapshot shape at ${path}: meta.horizon must be an ISO date string when present`,
    );
  }
  return snapshot;
}
 ...
 * returns null (open-phase-only fetch). A snapshot without a `meta.horizon`
 * falls back to the newest item fingerprint `updatedAt`. The overlap absorbs
 * clock skew between GitHub and the detector; re-fetched unchanged items diff
 * to zero deltas.
 */
export function horizonCutoff(oldSnapshot, overlapMs = HORIZON_OVERLAP_MS) {
  ...
  if (horizon != null && !isValidDateString(horizon)) {
    throw new Error(`invalid snapshot horizon: ${horizon}`);
  }
  if (!horizon) {
    for (const family of ['pr', 'issue']) {
      for (const item of Object.values(oldSnapshot[family] ?? {})) {
        const updatedAt = item?.fingerprint?.updatedAt;
        if (typeof updatedAt !== 'string') continue;
        if (!isValidDateString(updatedAt)) {
          throw new Error(`invalid snapshot horizon in ${family} fingerprint updatedAt`);
        }
        if (!horizon || updatedAt > horizon) {
          horizon = updatedAt;
        }
      }
    }
