...
import { economicalSnapshotPath, snapshotPath } from '../../snapshot.mjs';
...
function runReset(argv, deps = {}) {
  ...
  if (values['state-file']) {
    targets = [
      {
        ...
        logFile: resolve(deltaLogPath({ stateFile: values['state-file'] })),
      },
    ];
  } else {
    const watchStateFile = economicalSnapshotPath(
      repo.repo,
      monitorId,
      entities.key,
      values['state-dir'],
    ...
    targets = [
      {
        scope: 'poll',
        stateFile: snapshotPath(repo.repo, monitorId, entities.key, values['state-dir']),
        logFile: resolve(
          deltaLogPath({
      ...
      },
      {
        ...
        logFile: resolve(deltaLogPath({ stateFile: watchStateFile })),
      },
    ];
  }
  const lockOpts = { ghTimeoutMs: 60000, staleMs: 600000, fs: lockFs, now: lockNow };
  const locked = [];
  try {
    for (const target of [...targets].sort((a, b) => a.stateFile.localeCompare(b.stateFile))) {
      const acquired = acquireLock(target.stateFile, lockOpts);
      if (!acquired.ok)
        return errorResult(
          'busy',
          `state file locked (${acquired.reason}): ${target.stateFile}`,
          { at, command: 'reset' },
          format,
        );
      locked.push({ stateFile: target.stateFile, token: acquired.token });
    }
    const targetReports = targets.map((target) => {
      const snapshotRemoved = deleteSnapshot(target.stateFile);
      const logReset = resetDeltaLog(target.logFile);
      return {
        ...target,
        removed: [...(snapshotRemoved ? [target.stateFile] : []), ...logReset.removed],
        missing: [...(snapshotRemoved ? [] : [target.stateFile]), ...logReset.missing],
      };
    });
    return {
      code: 0,
      ...
      report: {
        schemaVersion: REPORT_SCHEMA_VERSION,
        command: 'reset',
        stateFile: targets[0].stateFile,
        logFile: targets[0].logFile,
        at,
        summary: `reset ${targetReports.flatMap((target) => target.removed).length} file(s)`,
    ...
    for (const lock of locked.reverse()) releaseLock(lock.stateFile, lock.token, lockOpts);
  }
}
