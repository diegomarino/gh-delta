...
- `comparableFingerprint` strips bookkeeping and detail-only keys
  ...
  `mergeStateStatus`) from a stored fingerprint before it is compared or hashed
  into a delta id.
...
| `mergeStateStatus`        | yes         | GitHub `mergeStateStatus` (`BEHIND` \| `BLOCKED` \| `CLEAN` \| `DIRTY` \| `DRAFT` \| `HAS_HOOKS` \| `UNSTABLE` \| `UNKNOWN`). Stored for the summary layer; **not** part of the change comparison or the delta id (stripped by `comparableFingerprint`, like `ciChecks` / `reviewSummary`). |
