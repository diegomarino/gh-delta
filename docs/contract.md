...
| `head`                    | opaque-ish  | head ref OID (git SHA). Treat as a change indicator; every push changes it. A transition emits `head-changed` (coexists with `updated`).                                                                                                                                                                                                                                                                                                           |
| `threadDigest`            | **opaque**  | sha1 digest of review-thread identity + resolution state (`{id, isResolved}` pairs). **Not part of `comparableFingerprint`/the delta id** — consulted directly by the detector as an additional change trigger so a same-count thread swap (one resolved, another reopened) is still observed. Absent from snapshots written before it was introduced (its first appearance does not fire).                                                        |
| `threadStates`            | yes         | sorted `{id, isResolved}` rows behind `threadDigest`, used to name which threads newly became unresolved/resolved. Same absence/upgrade rule as `threadDigest`, and likewise not part of the change comparison or the delta id.                                                                                                                                                                                                                    |
