...
| CI changed             | `PR_NUMBER=42; gh-delta wait --repo o/r --state-dir .state --timeout 30m --number "$PR_NUMBER" --until-summary ciRollup=green,failed` | On green, inspect review/mergeability; on failed, send the failing check to the worker.                        |
