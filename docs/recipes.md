...
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| PR merged              | `gh-delta --repo o/r --state-dir "${XDG_STATE_HOME:-$HOME/.local/state}/gh-delta/snapshots" --format compact`                                                                             | Verify the merge, then propose deleting its branch and advancing dependent work.                               |
| Review requested       | `gh-delta --repo o/r --format compact`                                                                                                                                                    | Notify the requested reviewer with the PR URL and current CI state.                                            |
| CI changed             | `PR_NUMBER=42; gh-delta wait --repo o/r --state-dir "${XDG_STATE_HOME:-$HOME/.local/state}/gh-delta/snapshots" --timeout 30m --number "$PR_NUMBER" --until-summary ciRollup=green,failed` | On green, inspect review/mergeability; on failed, send the failing check to the worker.                        |
...
| Changes requested      | `gh-delta --repo o/r --format compact`                                                                                                                                                    | Relay the requested changes to the worker; hold merge action.                                                  |
...
| Share one fetch        | `gh-delta --repo o/r --state-dir "${XDG_STATE_HOME:-$HOME/.local/state}/gh-delta/snapshots" --log --format compact`                                                                       | Let workers use separate `read --cursor` files; each cursor advances only after its own action.                |
