...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| PR merged              | `gh-delta --repo o/r --state-dir "${XDG_STATE_HOME:-$HOME/.local/state}/gh-delta" --format compact`                                                                             | Verify the merge, then propose deleting its branch and advancing dependent work.                               |
...
| CI changed             | `PR_NUMBER=42; gh-delta wait --repo o/r --state-dir "${XDG_STATE_HOME:-$HOME/.local/state}/gh-delta" --timeout 30m --number "$PR_NUMBER" --until-summary ciRollup=green,failed` | On green, inspect review/mergeability; on failed, send the failing check to the worker.                        |
...
| Share one fetch        | `gh-delta --repo o/r --state-dir "${XDG_STATE_HOME:-$HOME/.local/state}/gh-delta" --log --format compact`                                                                       | Let workers use separate `read --cursor` files; each cursor advances only after its own action.                |
